...
- Linter: Ruff.
- Type checker: Not configured in Stage 0; public service contracts should use explicit Pydantic models.
- Import/order rules: standard library, third-party, local imports in separate groups; avoid side-effect imports.
- Line length / docstring limits: 100 characters; docstrings explain public contracts and non-obvious constraints.
- Python-specific test rules: use pytest; mock external APIs and FFmpeg boundaries unless a test explicitly targets integration behavior.
...
- Migration conventions: Alembic migrations; never change schema without a migration once migrations exist.
...
- Shell dialect: Bash for local helper scripts.
